function BCCT_CON_matrixcomputemain_bat(Parameter)
pathfiles = which('BCCT_CON_matrixcomputemain.m');
...
if SeedROItype<3
    ...
    if GlobalScale
        ...
        for i = 1:size(datainput,2)
            DATtemp(DATMASK) = DATINPUTTEMP(DATMASK,i);
            DATtemp = DATtemp/mean(DATtemp);
            datainput(DATMASK,i) = DATtemp;
        end
